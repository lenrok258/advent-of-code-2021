...
input_rooms = (tuple(rooms_list[0]), tuple(rooms_list[1]), tuple(rooms_list[2]), tuple(rooms_list[3]) )
...
def find_free_spot_in_room(room_nr, rooms, amph_type):
    ...
    for spot_i in range(4):
        spot_v = room[spot_i]
        if spot_v[0] != '.' and spot_v[0] != amph_type:
            return None
...
def tick(hallway, rooms):
    ...
    for new_state in make_all_possible_moves(hallway, rooms):
        new_hallway, new_rooms, last_move_cost = new_state
        # new_costs = tick(new_hallway, new_rooms, cost + last_move_cost)
        new_costs = tick(new_hallway, new_rooms)
        new_costs_plus_last_move = list(map(lambda c: c+last_move_cost, new_costs))
        # print(new_costs_plus_last_move)
        all_possible_costs.extend(new_costs_plus_last_move)
...
result = tick(input_hallway, input_rooms)
